...
noakhaliDonateBtn.addEventListener("click", function () {
  ...
  const totalDonationAmount = parseFloat(
    document.getElementById("total-donation-available-Amount").innerText
  );
  document.getElementById("noakhali-input-field").value = "";
...
donationButton.addEventListener("click", function () {
  ...
  const mainBodySection = document.getElementById("main-section");
...
feniDonationBtn.addEventListener("click", function () {
  const totalAvailableAmount = getAvailableAmountId(
    "total-donation-available-Amount"
  );
...
quotaDonationBtn.addEventListener("click", function () {
  ...
  if (totalAmountQuota < quotaInputvalue && totalAmountQuota > 0) {
    alert("try again");
    document.getElementById("quota-btn").classList.add("hidden");
    window.location.reload();
    return;
  }
